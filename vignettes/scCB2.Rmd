...
Under R console, install package `scCB2` via GitHub (https://github.com/zijianni/scCB2):
...
#and you want to control FDR at 0.1%:
RealCell <-  QuickCB2(dir = "/path/to/raw/data/directory", FDR_threshold = 0.001, AsSeurat = FALSE)
...
The computational speed is related to the size and structure of input datasets. As a reference, scCB2 running on a medium-size dataset (around 8,000 cells and 20,000 genes) using 6 cores takes less than 10 minutes.
...
Under default parameters, we can directly use all-in-one function `QuickCB2` to get real cell matrix from 10x raw data. Note: Due to randomness and high-mitochondrial barcode filtering, `RealCell_Quick` is not the same as `RealCell`.
